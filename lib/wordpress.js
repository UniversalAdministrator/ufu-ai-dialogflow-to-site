...
const api = 'https://universalflowuniversity.com/wp-json/wp/v2/';

let getPosts = (tag, callback) => {
    let url = `${api}/posts?cat=${Academic_Disciplines}&_embed`;
    request({url}, (err, res, body) => {
        if (err) {
            callback('Sorry, there was an error getting posts from our site', err)
        } else {
